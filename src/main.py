...
def generate_slide(chapter, topic, output_pptx_path):
    try:
        ...
        return {"slides": [{"title": "Error", "content": "Failed to generate slide"}]}
    ...
    try:
        ...
        for topic in topics:
            ...
            if not os.path.exists(topic_file):
                ...
                logger.info(f"File {topic_file} already exists. Skipping regeneration.")
        logger.info(f"All slides have been successfully generated in folder {chapter_manager.chapter_folder}")
    except Exception as e:
        logger.error(f"Error generating chapter slides: {str(e)}")
